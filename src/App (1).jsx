...
export default function H3CRM() {
  ...
  // ---------- Formulário: Novo equipamento / Editar equipamento ----------
  const [showEquipmentModal, setShowEquipmentModal] = useState(false);
  const [editingEquipmentId, setEditingEquipmentId] = useState(null);
  const [newEquipment, setNewEquipment] = useState({ model: "", brand: "", serial: "", anvisa: "", status: "estoque" });

  const openEditEquipment = (e) => {
    setEditingEquipmentId(e.id);
    setNewEquipment({ model: e.model, brand: e.brand, serial: e.serial, anvisa: e.anvisa, status: e.status });
  ...
  const submitNewEquipment = async () => {
    ...
    try {
      if (editingEquipmentId) {
        const updated = await apiFetch(`/equipment/${editingEquipmentId}`, { method: "PUT", body: JSON.stringify(newEquipment) });
        setEquipment((es) => es.map((e) => (e.id === editingEquipmentId ? mapEquipmentFromApi(updated) : e)));
      } else {
  ...
  const deleteEquipment = async (id) => {
    if (!window.confirm("Excluir este equipamento? Essa ação não pode ser desfeita.")) return;
...
<body>
  ...
  return (
    <div className="w-full flex" style={{ background: colors.bg, minHeight: 600, fontFamily: "Inter, system-ui, sans-serif" }}>
        ...
        {view === "equipamentos" && (
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setEditingEquipmentId(null); setShowEquipmentModal(true); }} className="flex items-center gap-1.5 px-3 py-2 rounded-md text-sm text-white" style={{ background: colors.teal }}>
                  ...
                  {equipment.map((e) => {
                    ...
                    return (
                      <tr key={e.id} style={{ borderTop: `1px solid ${colors.border}` }}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <button onClick={() => openEditEquipment(e)} style={{ color: colors.teal }}>
                              <Pencil size={14} />
                            </button>
                            <button onClick={() => deleteEquipment(e.id)} style={{ color: colors.red }}>
                              <Trash2 size={14} />
                            </button>
                          </div>
                        </td>
      ...
      {showEquipmentModal && (
        <Modal title={editingEquipmentId ? "Editar equipamento" : "Novo equipamento"} onClose={closeEquipmentModal} onSubmit={submitNewEquipment} submitLabel={editingEquipmentId ? "Salvar alterações" : "Salvar"}>
          <Field label="Modelo">
            <input className="px-2.5 py-1.5 rounded-md text-sm" style={inputStyle}
